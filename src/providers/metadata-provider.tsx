...
export function MetadataProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <MetadataContext.Provider value={value}>
      <MetadataManager dynamicMetadata={dynamicMetadata} />
      {children}
...
type MetadataManagerProps = {
  dynamicMetadata?: CustomMetadataProps;
};
...
export function MetadataManager({
  ...
  const metadata = React.useMemo(() => {
    ...
    Object.entries(dynamicMetadata).forEach(([key, value]) => {
      if (value !== undefined) {
        merged[key as keyof CustomMetadataProps] = value;
      }
    });

    Object.entries(match.staticData).forEach(([key, value]) => {
      if (value !== undefined) {
        merged[key as keyof CustomMetadataProps] = value;
      }
    });

    if (!merged.twitterImage && merged.ogImage) {
      merged.twitterImage = merged.ogImage;
    }

    return merged;
  }, [dynamicMetadata, match.staticData]);
